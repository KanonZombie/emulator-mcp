...
description: >
  ...
  recur, harvest it without asking first. Uses a context-sharing sub-agent when
  available, otherwise works inline, to create a project-local or global SKILL.md.
license: MIT
metadata:
  author: kulaxyz
  version: "1.0"
...
- [ ] 2. **Dedupe.** Look for an existing skill to UPDATE rather than duplicate:
      the global skills directory and `<repo>/.agents/skills/`.
...
- [ ] 4. **Delegate the write** to a sub-agent that inherits this conversation
      (a fork/context-sharing agent), or do it inline if your client has no such
...
- **Project** (`<repo>/.agents/skills/`): the path is specific to THIS codebase
...
- **Global** (the client/Codex global skills directory): the path generalizes across projects — a
...
If your client permits a sub-agent that **inherits the current conversation**,
use it — it keeps harvesting out of the main context while seeing the golden
path. If it cannot or project rules forbid delegation, run the same steps
**inline** in the main loop. The rules below are identical either way.

Use a context-sharing fork, not a fresh contextless agent. The golden path only
exists in this conversation; a fresh agent would start blank. Sub-agents can
over-reach, so box the task in tightly. Use a prompt shaped like this:
